...
describe('POST /Admins', () => {
  test(
    'It should NOT create a new admin, FIRST NAME lenght did not pass validation',
    async () => {
...
describe('DELETE /admins', () => {
  test('response should return a false error, 200 status and successful message', async () => {
    const response = await request(app).patch(`/admins/${adminId}`).send();
  ...
  test('response should return an error, empty admin', async () => {
    const response = await request(app).patch('/admins/').send();
    expect(response.status).toBe(404);
  });

  test('response should return an error, bad path', async () => {
    const response = await request(app).patch('/asdasd').send();
    expect(response.status).toBe(404);
  });
});
